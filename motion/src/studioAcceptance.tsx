...
const Presenter=()=>{const f=useCurrentFrame();const idx=Math.min(147,Math.max(1,f+1));const file=`presenter-alpha/frame-${String(idx).padStart(4,'0')}.png`;return <div style={{position:'absolute',right:8,top:315,width:470,height:1205,overflow:'visible'}}><Img src={staticFile(file)} style={{position:'absolute',width:650,height:1155,right:-82,top:0,objectFit:'contain',filter:'drop-shadow(-14px 18px 28px rgba(0,0,0,.32))'}}/></div>};
