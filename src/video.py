...
def compose_vertical_video(
    ...
    for idx, (scene, audio) in enumerate(zip(story["scenes"], audio_files), start=1):
        # Audio is the master clock. Add a small tail so Arabic speech is never clipped.
        audio_duration = _duration(audio)
        duration = audio_duration + 0.35
        ...
        if suffix in {".jpg", ".jpeg", ".png", ".webp"}:
            vf = (
                ...
                "eq=contrast=1.04:saturation=0.96,format=yuv420p"
            )
            video_input = ["-loop", "1", "-i", str(visual)]
        else:
            vf = (
                "scale=1080:1920:force_original_aspect_ratio=increase,crop=1080:1920,"
                "fps=30,eq=contrast=1.03:saturation=0.98,format=yuv420p"
            )
            video_input = ["-stream_loop", "-1", "-i", str(visual)]

        # Do not use -shortest here: it was capable of ending the video stream before narration.
        _run([
    ...
    # Re-encode concat instead of stream-copying heterogeneous Pexels clips.
    _run([
    ...
    subtitle_filter = (
        f"subtitles='{sub}':force_style='FontName=Noto Sans Arabic,FontSize=19,"
